...
import { getCRClan } from '@/lib/api'
import { Bar, BarChart, CartesianGrid, Legend, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts'

export default function CRClanAnalysis() {
  const [clanTag, setClanTag] = useState('')
  const [searchTag, setSearchTag] = useState('')

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['crClan', clanTag],
    queryFn: () => getCRClan(clanTag),
    enabled: !!clanTag,
  })
  ...
  return (
    <Layout>
      <h1 className="text-3xl font-bold mb-6 text-center">Clash Royale Clan Analysis</h1>
        ...
        <div className="grid gap-8 md:grid-cols-2">
          <Card className="clash-card">
            ...
            <CardContent>
              ...
              <p><strong>Clan War Trophies:</strong> {clan.clanWarTrophies}</p>
              <p><strong>Required Trophies:</strong> {clan.requiredTrophies}</p>
              <p><strong>Donations Per Week:</strong> {clan.donationsPerWeek}</p>
